...
export class VehiclesService {
  ...
  async create(
    ...
    // Đảm bảo chỉ Owner và Nhanvien mới có quyền tạo xe và phải thuộc về một công ty
    if (
      (user.role_id !== ownerRoleId && user.role_id !== employeeRoleId) ||
      user.company_id === null
    ) {
      throw new ForbiddenException(
        'Bạn không có quyền tạo xe hoặc không thuộc về một công ty.',
      );
    }

    const { plate_number, vehicle_type_id, seat_layout_template_id } =
      createVehicleDto;
    ...
    if (existingVehicle) {
      throw new BadRequestException(
        `Xe với biển số '${plate_number}' đã tồn tại trong công ty của bạn.`,
      );
    }
    ...
    if (!vehicleType) {
      throw new NotFoundException(
        `Loại xe với ID ${vehicle_type_id} không tìm thấy.`,
      );
    }
    ...
    if (!seatLayoutTemplate) {
      throw new NotFoundException(
        `Sơ đồ ghế với ID ${seat_layout_template_id} không tìm thấy.`,
      );
    }
    // Logic này là chính xác: sơ đồ ghế phải thuộc công ty của user HOẶC là sơ đồ mặc định (company_id là null)
    if (
      seatLayoutTemplate.company_id !== user.company_id &&
      seatLayoutTemplate.company_id !== null
    ) {
      throw new BadRequestException(
        `Sơ đồ ghế với ID ${seat_layout_template_id} không thuộc về công ty của bạn hoặc không phải sơ đồ mặc định.`,
      );
    }
  ...
  async findAll(user: AuthenticatedUser): Promise<vehicles[]> {
    ...
    // OWNER va NHANVIEN chỉ xem xe của công ty mình
    else if (user.role_id === ownerRoleId || user.role_id === employeeRoleId) {
  ...
  async update(
    ...
    if (user.role_id === ownerRoleId || user.role_id === employeeRoleId) {
      if (
        user.company_id === null ||
        existingVehicle.company_id !== user.company_id
      ) {
    ...
    if (
      ...
      if (duplicateVehicle) {
        throw new BadRequestException(
          `Xe với biển số '${updateVehicleDto.plate_number}' đã tồn tại trong công ty này.`,
        );
      }
    }
    ...
    if (
      updateVehicleDto.vehicle_type_id &&
      updateVehicleDto.vehicle_type_id !== existingVehicle.vehicle_type_id
    ) {
      const vehicleType = await this.prisma.vehicle_types.findUnique({
      ...
      if (!vehicleType) {
        throw new NotFoundException(
          `Loại xe với ID ${updateVehicleDto.vehicle_type_id} không tìm thấy.`,
        );
      }
    }
    ...
    if (
      updateVehicleDto.seat_layout_template_id &&
      updateVehicleDto.seat_layout_template_id !==
        existingVehicle.seat_layout_template_id
    ) {
      const seatLayoutTemplate =
        await this.prisma.seat_layout_templates.findUnique({
      ...
      if (!seatLayoutTemplate) {
        throw new NotFoundException(
          `Sơ đồ ghế với ID ${updateVehicleDto.seat_layout_template_id} không tìm thấy.`,
        );
      }
      ...
      if (
        seatLayoutTemplate.company_id !== existingVehicle.company_id &&
        seatLayoutTemplate.company_id !== null
      ) {
        throw new BadRequestException(
          `Sơ đồ ghế với ID ${updateVehicleDto.seat_layout_template_id} không thuộc về công ty của xe hoặc không phải sơ đồ mặc định.`,
        );
      }
    }
