...
export class VehiclesController {
  ...
  @Post()
  @Roles(UserRole.OWNER) // Chỉ Owner có thể tạo xe
  @ApiOperation({ summary: 'Tạo một xe mới cho công ty của Owner (Owner Only)' })
  @ApiResponse({ status: 201, description: 'Xe đã được tạo thành công.' })
  @ApiResponse({ status: 400, description: 'Dữ liệu không hợp lệ hoặc biển số xe đã tồn tại, ID không hợp lệ.' })
  ...
  async create(@Body() createVehicleDto: CreateVehicleDto, @Req() req: Request) {
    ...
    return this.vehiclesService.create(createVehicleDto, req.user as AuthenticatedUser);
  }

  @Get()
  @Roles(UserRole.ADMIN, UserRole.OWNER) // Admin xem tất cả, Owner xem của mình
  @ApiOperation({ summary: 'Lấy tất cả các xe (Admin xem tất cả, Owner xem của công ty mình)' })
  ...
  @Get(':id')
  @Roles(UserRole.ADMIN, UserRole.OWNER) // Admin xem bất kỳ, Owner xem của mình
  @ApiOperation({ summary: 'Lấy một xe theo ID (Admin xem bất kỳ, Owner xem của công ty mình)' })
  ...
  @Patch(':id')
  @Roles(UserRole.ADMIN, UserRole.OWNER) // Admin cập nhật bất kỳ, Owner cập nhật của mình
  @ApiOperation({ summary: 'Cập nhật thông tin xe (Admin cập nhật bất kỳ, Owner cập nhật của công ty mình)' })
  @ApiResponse({ status: 200, description: 'Xe đã được cập nhật thành công.' })
  @ApiResponse({ status: 400, description: 'Dữ liệu không hợp lệ hoặc lỗi liên kết ID.' })
  @ApiResponse({ status: 403, description: 'Không có quyền truy cập.' })
  @ApiResponse({ status: 404, description: 'Không tìm thấy xe.' })
  async update(@Param('id') id: string, @Body() updateVehicleDto: UpdateVehicleDto, @Req() req: Request) {
    ...
    return this.vehiclesService.update(+id, updateVehicleDto, req.user as AuthenticatedUser);
  }

  @Delete(':id')
  @HttpCode(HttpStatus.NO_CONTENT) // 204 No Content cho delete thành công
  @Roles(UserRole.ADMIN, UserRole.OWNER) // Admin xóa bất kỳ, Owner xóa của mình
  @ApiOperation({ summary: 'Xóa một xe (Admin xóa bất kỳ, Owner xóa của công ty mình)' })
  @ApiResponse({ status: 204, description: 'Xe đã được xóa thành công.' })
  @ApiResponse({ status: 400, description: 'Không thể xóa vì xe đang được sử dụng.' })
  @ApiResponse({ status: 403, description: 'Không có quyền truy cập.' })
  @ApiResponse({ status: 404, description: 'Không tìm thấy xe.' })
