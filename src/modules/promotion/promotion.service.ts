...
export class PromotionService {
    ...
    async create(dto: CreatePromotionDto, user: AuthenticatedUser): Promise<promotions> {
        // Kiểm tra code đã tồn tại chưa
        const existing = await this.prisma.promotions.findUnique({ where: { code: dto.code } });
        if (existing) {
        ...
        if (user.role_id === 2) { // Role Company
            if (!user.company_id) {
        ...
        } else if (user.role_id !== 1) { // Role Admin = 1
        ...
        try {
            ...
            throw new BadRequestException('Không thể tạo khuyến mãi. Dữ liệu không hợp lệ.');
        }
    }
    ...
    async findAll(query: FindPromotionsQueryDto, user: AuthenticatedUser) {
        ...
        } else if (user.role_id === 1) { // Role Admin
        ...
        const [promotions, total] = await this.prisma.$transaction([
            this.prisma.promotions.findMany({
                ...
                include: { // Include tên nhà xe nếu có
    ...
    async update(id: number, dto: UpdatePromotionDto, user: AuthenticatedUser): Promise<promotions> {
        ...
        if (dto.code && dto.code !== existingPromotion.code) {
            const conflict = await this.prisma.promotions.findUnique({ where: { code: dto.code } });
            if (conflict) {
                throw new ConflictException(`Mã khuyến mãi "${dto.code}" đã tồn tại.`);
            }
        }
        ...
        try {
            ...
            throw new BadRequestException('Không thể cập nhật khuyến mãi. Dữ liệu không hợp lệ.');
        }
    }
    ...
    async remove(id: number, user: AuthenticatedUser): Promise<{ message: string }> {
        ...
        try {
            ...
            throw new BadRequestException(`Không thể xóa khuyến mãi ID ${id}. Có thể đang được sử dụng.`);
        }
    }
    ...
    async validatePromotion(
    ...
    ): Promise<Pick<promotions, 'id' | 'description' | 'discount_type' | 'discount_value'>> {
        const now = new Date();
        ...
        // --- KIỂM TRA MỚI: Nhà xe ---
        if (promotion.company_id !== null && promotion.company_id !== bookingCompanyId) {
            throw new ForbiddenException(`Mã khuyến mãi "${code}" không áp dụng cho nhà xe này.`);
        }

        // --- Các kiểm tra cũ ---
        if (!promotion.is_active) {
            throw new BadRequestException(`Mã khuyến mãi "${code}" hiện không được kích hoạt.`);
        }
        if (promotion.valid_from > now) {
            throw new BadRequestException(`Mã khuyến mãi "${code}" chưa đến ngày áp dụng.`);
        }
        ...
        if (promotion.usage_limit !== null) {
            ...
            if (usageCount >= promotion.usage_limit) {
                throw new BadRequestException(`Mã khuyến mãi "${code}" đã hết lượt sử dụng.`);
            }
        }
     ...
     */
    async findApplicable(query: FindApplicablePromotionsQueryDto): Promise<Omit<promotions, 'usage_limit' | 'created_at' | 'is_active'>[]> {
        ...
        const applicablePromotions = await this.prisma.promotions.findMany({
            ...
            select: { // Chỉ lấy các trường cần thiết cho frontend hiển thị
                id: true,
                code: true,
                description: true,
                image_url: true,
     ...
     */
    async findPublic(limit: number = 5): Promise<Omit<promotions, 'usage_limit' | 'created_at' | 'is_active'>[]> {
        const now = new Date();
