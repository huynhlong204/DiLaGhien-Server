...
export class TripsController {
  ...
  @ApiResponse({ status: 201, description: 'Chuyến đi đã được tạo thành công.' })
  @ApiResponse({ status: 400, description: 'Dữ liệu không hợp lệ.' })
  @ApiResponse({ status: 403, description: 'Không có quyền.' })
  @ApiResponse({ status: 404, description: 'Không tìm thấy tài nguyên liên quan.' })
  ...
  @ApiResponse({ status: 201, description: 'Các chuyến đi định kỳ đã được tạo thành công.' })
  @ApiResponse({ status: 400, description: 'Dữ liệu không hợp lệ hoặc thiếu thông tin định kỳ.' })
  @ApiResponse({ status: 403, description: 'Không có quyền.' })
  @ApiResponse({ status: 404, description: 'Không tìm thấy tài nguyên liên quan.' })
  async createRecurring(@Body() createRecurringTripDto: CreateRecurringTripDto, @Req() req): Promise<any[]> {
    const user: AuthenticatedUser = req.user;
    return this.tripService.createRecurring(createRecurringTripDto, user);
  }

  @Get()
  @Roles(UserRole.ADMIN, UserRole.OWNER, UserRole.DRIVER, UserRole.PASSENGER) // Passenger có thể xem các chuyến đi công khai
  ...
  @Get(':id')
  @Roles(UserRole.ADMIN, UserRole.OWNER, UserRole.DRIVER, UserRole.PASSENGER)
  ...
  @ApiResponse({ status: 200, description: 'Chuyến đi đã được cập nhật thành công.' })
  @ApiResponse({ status: 400, description: 'Dữ liệu không hợp lệ hoặc không thể cập nhật.' })
  @ApiResponse({ status: 403, description: 'Không có quyền.' })
  @ApiResponse({ status: 404, description: 'Chuyến đi không tìm thấy.' })
  async update(@Param('id', ParseIntPipe) id: number, @Body() updateTripDto: UpdateTripDto, @Req() req): Promise<any> {
    const user: AuthenticatedUser = req.user;
  ...
  @ApiResponse({ status: 200, description: 'Trạng thái chuyến đi đã được cập nhật.' })
  @ApiResponse({ status: 400, description: 'Trạng thái không hợp lệ hoặc chuyển đổi trạng thái không cho phép.' })
  @ApiResponse({ status: 403, description: 'Không có quyền.' })
  @ApiResponse({ status: 404, description: 'Chuyến đi không tìm thấy.' })
  async updateStatus(@Param('id', ParseIntPipe) id: number, @Body('status') status: TripStatus, @Req() req): Promise<any> {
    const user: AuthenticatedUser = req.user;
  ...
  @ApiResponse({ status: 204, description: 'Chuyến đi đã được xóa thành công.' })
  @ApiResponse({ status: 400, description: 'Không thể xóa chuyến đi (ví dụ: đã có vé).' })
  @ApiResponse({ status: 403, description: 'Không có quyền.' })
  @ApiResponse({ status: 404, description: 'Chuyến đi không tìm thấy.' })
  ...
  @Roles(UserRole.ADMIN, UserRole.OWNER, UserRole.PASSENGER)
  ...
  async getTripsByCompanyRoute(
    ...
    if (!isValidDate) {
      throw new BadRequestException('Tham số "date" phải đúng định dạng YYYY-MM-DD.');
    }
