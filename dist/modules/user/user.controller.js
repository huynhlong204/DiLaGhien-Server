...
let UserController = class UserController {
    ...
    findAll(roleId, companyId) {
        const filterOptions = {};
        if (roleId) {
            filterOptions.role_id = parseInt(roleId, 10);
            if (isNaN(filterOptions.role_id)) {
                throw new common_1.BadRequestException('Role ID phải là số hợp lệ.');
            }
        }
        if (companyId) {
            filterOptions.company_id = parseInt(companyId, 10);
            if (isNaN(filterOptions.company_id)) {
                throw new common_1.BadRequestException('Company ID phải là số hợp lệ.');
            }
        }
...
__decorate([
    (0, common_1.Get)(),
    __param(0, (0, common_1.Query)('role_id')),
